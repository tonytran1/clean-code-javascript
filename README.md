...
### Use ES6 constants when variable values are not reassigned.
In the bad example, the variable value can be reassigned.
When you assign a constant value, the value stays the same throughout
the program.

Note: Using const only limits declared const primitive values from being reassigned.
If an object or array is assigned to a const, referenced objects can be changed
and are mutable. See the example below.

**Bad:**
```javascript
var FIRST_US_PRESIDENT = "George Washington"; // Can be reassigned
```

**Good**:
```javascript
const FIRST_US_PRESIDENT = "George Washington"; // Cannot be reassigned

// The following will not cause any errors:
const presidents = { current : "Barack Obama" };
presidents.current = "Donald Trump";
```
**[⬆ back to top](#table-of-contents)**
